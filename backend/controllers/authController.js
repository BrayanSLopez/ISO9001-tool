...
router.post('/register', (req, res) => {
  ...
  const {
    ...
    direccion, telefono, num_empleados, email_empresa, web, facebook, instagram, tiktok
  } = req.body;

  // Crear empresa primero
  db.run(`INSERT INTO empresas (razon_social,nit,representante_legal,sector_economico,tipo_empresa,direccion,telefono,num_empleados,email,web,facebook,instagram,tiktok)
    VALUES (?,?,?,?,?,?,?,?,?,?,?,?)`,
    [razon_social, nit, representante_legal, sector_economico, tipo_empresa, direccion, telefono, num_empleados, email_empresa, web, facebook, instagram, tiktok],
    function (err) {
      if (err) {
        db.close();
      ...
      bcrypt.hash(password, 10, (errh, hash) => {
        ...
        db.run(`INSERT INTO usuarios (nombre, correo, password, empresa_id) VALUES (?,?,?,?)`,
...
router.post('/login', (req, res) => {
  ...
  db.get('SELECT * FROM usuarios WHERE correo = ?', [correo], (err, user) => {
    ...
    bcrypt.compare(password, user.password, (errc, ok) => {
      ...
      const token = jwt.sign({ id_usuario: user.id_usuario, correo: user.correo }, SECRET, { expiresIn: '12h' });
      res.json({ message: 'Login OK', token, user: { id_usuario: user.id_usuario, nombre: user.nombre, correo: user.correo } });
    });
  });
});
